...
private:
    ...
    typename Map<KeyType, ValueType>::Node* m_head;
    int   m_size;
    
    typename Map<KeyType, ValueType>::Node* find(const KeyType& key) const;
...
{
    // create dummy node
    m_head = new typename Map<KeyType, ValueType>::Node;
...
Map<KeyType, ValueType>::~Map()
{
    ...
    for (typename Map<KeyType, ValueType>::Node* p = m_head->m_prev ; m_size >= 0; m_size--)
    {
        typename Map<KeyType, ValueType>::Node* toBeDeleted = p;
        p = p->m_prev;
        delete toBeDeleted;
    }
}
...
: m_size(other.m_size)
{
    ...
    typename Map<KeyType, ValueType>::Node* prev = m_head;
    
    // Copy each non-dummy node from the other list; each iteration will set
    // the m_next of the previous node copied
    
    for (typename Map<KeyType, ValueType>::Node* p = other.m_head->m_next ; p != other.m_head; p = p->m_next)
    {
        // Create a copy of the node p points to
        typename Map<KeyType, ValueType>::Node* pnew = new typename Map<KeyType, ValueType>::Node;
        pnew->m_key = p->m_key;
...
{
    typename Map<KeyType, ValueType>::Node* p = find(key);
...
bool Map<KeyType, ValueType>::get(int i, KeyType& key, ValueType& value) const
{
    ...
    typename Map<KeyType, ValueType>::Node* p;
...
{
    // swap head pointers
    typename Map<KeyType, ValueType>::Node* tempHead = m_head;
    m_head = other.m_head;
                                               ...
                                               bool mayInsert, bool mayUpdate)
{
    ...
    // Create a new node
    p = new typename Map<KeyType, ValueType>::Node;
    p->m_key = key;
    p->m_value = value;
